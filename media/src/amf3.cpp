...
DWORD AMFParser::Parse(BYTE *data,DWORD size)
{
	...
	switch((TypeMarker)data[0])
	{
		...
		case AvmplusObjectMarker:
			//Create the object
			return Error("AVMPlus Not supported");
		default:
			//Create the object
			throw std::runtime_error(std::string("Type Not supported"));
	}

	//If we have got an object
	if (object)
		//Debug
		Debug("Got %d %s data type\n",data[0],object->GetTypeName());
...
void AMFNumber::SetNumber(double d)
{
	if (!d)
	{
		...
		value = 0x7FF0000000000000LL + ((int64_t)(d<0)<<63) + (d!=d);
	} else {
		int e;
		double f= frexp(d, &e);
		value = (int64_t)(f<0)<<63 | (e+1022LL)<<52 | (int64_t)((fabs(f)-0.5)*(1LL<<53));
	}
}
...
void AMFObject::AddProperty(const wchar_t* key,AMFObject *obj)
{
	//Set property
	properties[key] = (AMFData*)obj;
}
...
DWORD AMFEcmaArray::Parse(BYTE *data,DWORD size)
{
	...
	while (bufferSize)
	{
		...
		//Check if we are processing the utf 8 size of the property name
		if (len<2)
		{
			//Append the byte to the string size
			((BYTE*)&utf8size)[len++] = buffer[0];
			//Remove from buffer
			buffer++;
			bufferSize--;
			//Continue processing
		...
		//Check if got the size of the utf8 property name
		if (len==2)
		{
			...
			{
				//Set the size to the utf8 parser
				utf8parser.SetSize(utf8size);
			} else if (bufferSize) {
				//CCheck if next if the end marker
				if (((AMFParser::TypeMarker)buffer[0])!=AMFParser::ObjectEndMarker)
					//Error should not happend finish parsing 
		...
		//Check if we are still parsing the utf8 property name
		if (len-2<utf8size)
		{
			//Parse utf8 string
			copy = utf8parser.Parse(buffer,bufferSize);
		...
		{
			//Append the property to the map
			elements[utf8parser.GetWString()] = parser.GetObject();
			//Start parsing next property
			utf8parser.Reset();
			len = 0;
		}
	}
...
DWORD AMFStrictArray::Parse(BYTE *data,DWORD size)
{
	...
	while (bufferSize)
	{
		...
		if (len==4)
		{
			...
			{
				//Allocate memory
				elements = (AMFData**)malloc(num*sizeof(AMFObject*));
				//Set first
				cur = 0;
			}
		}

...
{
	Debug("[RTMPMessage type:%d,streamId:%u,timestamp:%llu,%length:%u]\n",TypeToString(type),streamId,timestamp,length);
	if (ctrl)
		 ctrl->Dump();
	if (cmd)
		 cmd->Dump();
...
DWORD RTMPCommandMessage::Serialize(BYTE* buffer,DWORD size)
{
	...
	for(DWORD i=0;i<extra.size();i++)
		len += extra[i]->Serialize(buffer+len,size-len);
...
RTMPVideoFrame::RTMPVideoFrame(QWORD timestamp,AVCDescriptor *desc) : RTMPMediaFrame(Video,timestamp, desc->GetSize())
{
	//Set type

...
bool RTMPFLVStream::Play(std::wstring& url)
{
	...
	//Send play comand
	playListener->OnCommand(id,L"onStatus", new RTMPNetStatusEvent(L"NetStream.Play.Reset",L"status",L"Playback reset") );
...
void RTMPFLVStream::PublishMediaFrame(RTMPMediaFrame *frame)
{
	...
	//Write frame
	write(fd,frame->GetData(),frame->GetSize());
...
int RTMPFLVStream::PlayFLV()
{
	...
	//Send stream is Recoreded
	playListener->OnStreamIsRecorded(id);
	//Send stream begin

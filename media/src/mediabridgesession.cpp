...
bool MediaBridgeSession::SetTransmitter(RTMPStreamPublisher *transmitter)
{
	Log("-Set transmitter\n");
...
void MediaBridgeSession::UnsetTransmitter()
{
	Log(">UnsetTransmitter\n");
...
bool MediaBridgeSession::OnPublishedCommand(DWORD streamId,const wchar_t* name,AMFData* obj)
{
	return false;
}

bool MediaBridgeSession::OnPublishedFrame(DWORD streamId,RTMPMediaFrame* frame)
{
...
bool MediaBridgeSession::SetReceiver(RTMPStream *receiver)
{
	//Lock
	pthread_mutex_lock(&mutex);

	//Check if we already have a receiver for it
	if (this->receiver || !inited || !receiver)
	{
		//Unlock
		pthread_mutex_unlock(&mutex);
		//error
...
bool MediaBridgeSession::UnsetReceiver()
{
	Log(">UnsetReceiver\n");
...
bool MediaBridgeSession::OnPublishedMetaData(DWORD streamId,RTMPMetaData *publishedMetaData)
{
	//Cheeck
	if (!sendingText)
		//Exit

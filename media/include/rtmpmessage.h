...
public:
	...
	RTMPVideoFrame(QWORD timestamp,AVCDescriptor *desc);
	...
	VideoCodec	GetVideoCodec()				{ return codec;			}
	FrameType	GetFrameType()				{ return frameType;		}
	BYTE		GetAVCType()				{ return extraData[0];		}
	DWORD		GetAVCTS()				{ return ((DWORD)extraData[1]) << 16 | ((DWORD)extraData[2]) << 8 | extraData[3]; }
